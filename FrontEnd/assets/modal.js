...
/**
 * Gère la prévisualisation de la photo sélectionnée
 */
...
/**
 * Gère le bouton valider qui est grisé tant que le formulaire n'est pas totalement rempli
 */
function handleSubmitButton() {
    ...
    function checkFormValidity() {
        if (fileInput.files.length > 0 && titleInput.value.trim() !== '') { //trim() pour s'assurer que l'utilisateur n'as pas saisie que des espaces
...
/**
 * Gère l'affichage du formulaire d'envoi de photo
 */
async function handleAddForm() {
    previewImageFile();
    await displayModalCategories(await getCategories());
    handleSubmitButton();
...
/**
 * 
 * @param {HTMLElement} button Cache les boutons
 */
...
/**
 * 
 * @param {HTMLElement} button Affiche les boutons
 */
...
export function handleModal() {
    ...
    // Écoute des boutons "modifier" afin d'ouvrir la fenêtre de dialogue
    ...
    // Écoute du bouton X afin de fermer la fenêtre de dialogue
    ...
    // Écoute de la flèche de retour 
    backButton.addEventListener("click", async () => {
        injectTemplate("modal-gallery-layout");
        await displayModalGallery(await getWorks());
        hideButton(backButton);
        displayButton(addButton);
    });
